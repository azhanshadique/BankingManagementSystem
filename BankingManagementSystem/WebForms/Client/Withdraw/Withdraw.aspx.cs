...
namespace BankingManagementSystem.WebForms.Client.Withdraw
{
    public partial class Withdraw : System.Web.UI.Page
    {
        ...
        protected async void BtnWithdraw_Click(object sender, EventArgs e)
        {
            ...
            var result = await ClientService.WithdrawAmountAsync(withdrawDto);
            string message = GetParsedErrorMessage(result.Message);
            if (result.IsSuccess)
            {

                string redirectUrl = ResolveClientUrl(Page.GetRouteUrl("WithdrawAmountRoute", null));
                string script = $@"
                    setTimeout(function() {{
                        showDynamicModal({{
                            titleText: 'Amount Withdrawn',
